...
│  IP: 192.168.100.1/24            │
│  NAT masquerade ether1 → internet│
│  DHCP server propio: DESHABILITADO│
└──┬──────┬──────┬──────┬──────────┘
   │      │      │      │
...
| Dispositivo | IP | Rol |
|---|---|---|
| MikroTik (bridge-lan) | `192.168.100.1/24` | Gateway del segmento + NAT hacia el modem |
| Raspberry Pi (master) | `192.168.100.2/24` (estática) | dnsmasq (DHCP+PXE) + nginx (HTTP) + sync-server (persistencia) |
| Laptop / PCs cliente | DHCP, rango `192.168.100.100`-`192.168.100.200` | Arrancan HuronOS por red |

El gateway se dejó deliberadamente en `192.168.100.1` — el mismo valor que ya usa `dnsmasq.conf` de la simulación — así solo hace falta cambiar la IP propia del master (`192.168.100.2`) y el nombre de la interfaz, no el gateway.

## 3. Configuración del MikroTik (RouterOS)

Conéctate por Winbox/WebFig o SSH al MikroTik con su config de fábrica y corre esto en la terminal (ajusta `ether1` si tu puerto WAN físico es otro):

```
# Bridge para los puertos LAN (ether2-5)
...
# IP del bridge (gateway del segmento de examen)
/ip address add address=192.168.100.1/24 interface=bridge-lan

# ether1 como WAN: pide IP al modem por DHCP
/ip dhcp-client add interface=ether1 disabled=no

# NAT: todo lo que sale de bridge-lan hacia ether1 se enmascara
/ip firewall nat add chain=srcnat out-interface=ether1 action=masquerade

# IMPORTANTE: apagar el DHCP server de fábrica del hEX lite en el bridge LAN
# (el hEX lite normalmente trae uno activo en ether2-5 con IP 192.168.88.1;
# si sigue activo, competirá con el dnsmasq de la RPi)
/ip dhcp-server print
# Anota el nombre del server que aparezca sobre bridge-lan/ether2-5 y deshabilítalo:
/ip dhcp-server disable [find interface=bridge-lan]
# Si el server de fábrica está sobre una interfaz distinta (ej. "ether2-master-local"),
# ajusta el "interface=" del comando anterior al nombre real que viste en el print.
```

Verifica desde el propio MikroTik:

```
/ip address print          # debe mostrar 192.168.100.1/24 en bridge-lan
/ip dhcp-client print       # debe mostrar una IP asignada por el modem en ether1
/ip dhcp-server print       # el que esté sobre bridge-lan debe decir "disabled=yes"
```

## 4. Configuración de red de la Raspberry Pi

IP estática `192.168.100.2/24`, gateway `192.168.100.1`, DNS `8.8.8.8` (o el que prefieras).

Primero identifica qué maneja la red en tu RPi:

```bash
cat /etc/os-release
systemctl status NetworkManager   # si está "active", usa la opción A
```

**Opción A — NetworkManager (Raspberry Pi OS Bookworm o más nuevo):**

```bash
nmcli connection show                       # identifica el nombre de la conexión Ethernet, ej. "Wired connection 1"
...
**Opción B — `dhcpcd` (Raspberry Pi OS más antiguo):**

Agregar al final de `/etc/dhcpcd.conf` (ajustando `eth0` si tu interfaz se llama distinto):
...
Luego `sudo systemctl restart dhcpcd`.

Confirma con `ip addr show` que la IP quedó asignada correctamente antes de seguir.
...
Busca la interfaz Ethernet conectada al MikroTik (normalmente `eth0`, pero en instalaciones nuevas de Raspberry Pi OS puede llamarse `end0`). Si NO es `eth0`, edita `experimento_hardware_real/master/dnsmasq.conf` en este repo (la línea `interface=eth0`) antes de construir la imagen.
...
# 2. Usar el boot.ipxe adaptado a hardware real (server=192.168.100.2)
...
Nota: `boot/boot.ipxe` normalmente está versionado con `set server 192.168.100.1` (valor de la simulación). El paso 2 lo sobreescribe para hardware real — si vuelves a correr la simulación QEMU después, recuerda restaurarlo con `git checkout boot/boot.ipxe`.
...
1. **DHCP**: la laptop debe recibir una IP en `192.168.100.100-200` con gateway `192.168.100.1`. Revisa `docker logs -f ipxe-master` en la RPi (verás el `log-dhcp` de `dnsmasq`) mientras arranca la laptop.
2. **HTTP**: desde la RPi y desde cualquier otra PC del segmento:
   ```bash
   curl http://192.168.100.2/boot.ipxe
   curl -I http://192.168.100.2/huronos-system.sfs
   ```
...
| `boot/boot.ipxe` | `set server 192.168.100.1` (el propio host) | `set server 192.168.100.2` (la RPi) — ver `experimento_hardware_real/boot.ipxe` |
